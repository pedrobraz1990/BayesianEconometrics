...
def KalmanFilter(y, nStates, Z, H, T, Q, a1, P1, R):
    ...
    for t in range(0, n - 1):
        ...
        # if True:
            ...
            Ft[:, :, t] = np.dot(np.dot(Z,P[:,:,t]),ZT) + H
            # Ft[:,:,t] = (Z.dot(P[:,:,t]).dot(ZT) + H)

            Finv = inv(Ft[:,:,t])


            a[:, t] = a[:, t] + np.dot(np.dot(np.dot(P[:,:,t],ZT),Finv),vt[t,:])
            # a[:,t] = a[:,t] + P[:,:,t].dot(ZT).dot(Finv).dot(vt[t,:])


            P[:, :, t] = P[:, :, t] - np.dot(np.dot(np.dot(np.dot(P[:,:,t],ZT),Finv),Z),P[:,:,t])
            ...
            P[:, :, t + 1] = np.dot(np.dot(T,P[:,:,t]),TT) + np.dot(np.dot(R,Q),RT)
            ...
            Hst = np.dot(np.dot(select,H),select.T)
            # Hst = select.dot(H).dot(select.T)

            vt[t,ind2] = yst - np.dot(Zst, a[:,t])

            Ft[:dim,:dim,t] = np.dot(np.dot(Zst,P[:, :,t]),ZstT) + Hst
            # Ft[:dim,:dim,t] = Zst.dot(P[:, :,t]).dot(ZstT) + Hst

            Finv = inv(Ft[:dim, :dim, t])

            a[:,t] = a[:,t] + np.dot(np.dot(np.dot(P[:, :,t],ZstT),Finv),vt[t,ind2])
            # a[:,t] = a[:,t] + P[:, :,t].dot(ZstT).dot(Finv).dot(vt[t,ind2])

            P[:, :,t] = P[:, :,t] - np.dot(np.dot(np.dot(np.dot(P[:, :,t],ZstT),Finv),Zst),P[:, :,t])
            ...
            yhat[t, ind2] = np.dot(Zst,a[:, t])
            # yhat[t, ind2] = Zst.dot(a[:, t])
            yhat[t ,~ind2] = np.dot(Z,a[:,t])[~ind2]
    ...
    for t in range(0, n - 1):
        ...
        if ind[t] < 2:
            ll += np.log(det(Ft[:dims[t],:dims[t],t])) + np.dot(np.dot(vt[t,inds[t,:]].T,inv(Ft[:dims[t],:dims[t],t])),vt[t,inds[t,:]])
